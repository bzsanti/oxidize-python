...
[2026-04-21] Publicamos un reader RAG a PyPI sin auditar el comportamiento semántico

- Qué hice mal: construí `llama-index-readers-oxidize-pdf` sobre `rag_chunks()` y publiqué a PyPI + abrí issue pidiendo listing en LlamaHub, con claims de "RAG-ready chunking" y "semantic chunks" en README + issue body. Los 22 tests unitarios solo verificaban shape (existencia de claves, tipos), no contenido semántico. Ningún test comprobó que los chunks fueran disjoint, que heading_context se poblara consistentemente, ni que element_types fuera algo útil además de una lista.
- Causa raíz: confié en los nombres de las APIs del bridge (`rag_chunks`, `heading_context`, `element_types`) como garantía de que el comportamiento subyacente cumplía lo prometido. Confundí "existe la clave en el dict" con "la semántica es correcta". No escribí tests que compararan input conocido vs output esperado semánticamente.
- Cómo lo solucioné: auditoría empírica con PDFs de estructura conocida, destapé que `rag_chunks()` emite chunks acumulativos (cada chunk contiene al anterior + un elemento). Pause de claims, documentación del bug en memoria, prompt de fix preparado para el core.
- Regla para evitarlo: antes de publicar un adapter que promete capacidades semánticas (chunking, clasificación, detección de headings, etc.), escribir al menos un test de "known input → expected semantic output" que verifique el comportamiento, no solo la shape. Smoke tests de shape son insuficientes para adapters — aunque los datos fluyan, pueden fluir basura. Para publicación pública, el listón mínimo es: "¿un usuario haciendo lo mismo que hace mi test obtendría lo que yo prometo?"
